...
async def get_cards(category, page=1, per_page=3):
    print(f"Fetching case studies for category: {category}, page: {page}")
    ...
    try:
        ...
        cards = []
        for record in case_studies_records:
            case_study_data = record['case_study']
            cards.append(
                f"""<a href="/case_study/{record['slug']}" class="block">
                        <div class="card bg-white rounded-xl shadow-md overflow-hidden flex-1 hover:shadow-lg transition-shadow duration-300">
                            <div class="h-48 overflow-hidden">
                                <img src="{case_study_data.get('mainImageUrl', 'https://picsum.photos/seed/default/800/400')}" alt="{case_study_data.get('mainImageAlt', 'Case Study Image')}" class="w-full h-full object-cover">
                            </div>
                            <div class="p-6">
                                <h3 class="text-xl font-bold text-gray-800 mb-2">{case_study_data.get('blogTitle', 'Case Study Title')}</h3>
                                <p class="text-gray-600 mb-4">{case_study_data.get('blogSummary', 'Case study summary.')}</p>
                                <div class="flex items-center text-sm text-gray-500 mb-3">
                                    <span>{case_study_data.get('author', 'Suflex Media')}</span>
                                    <span class="mx-2">•</span>
                                    <span>{case_study_data.get('blogDate', 'N/A')}</span>
                                </div>
                            </div>
                        </div>
                    </a>"""
...
async def get_more_case_studies_section(data: dict):
    template = """
    <style>
        more_case_studies {
    ...
    <section class="py-12 px-4 more_case_studies">
    <hr class="border-t border-black my-8 md:my-12 w-full md:w-[90%] lg:w-[80rem] mx-auto" />
    <div class="max-w-6xl mx-auto">
        <!-- Heading -->
        <h2 class="text-4xl font-serif text-center mb-10 text-gray-800">Related Case Studies</h2>
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="flex justify-center mt-8">
                <a href="/case-studies" class="see-more-btn px-8 py-3 text-white font-medium rounded-lg inline-flex items-center gap-2">
    ...
    return template.replace("[[cards]]", await get_cards(data.get("caseStudyCategory", "sample_category")))


async def get_case_study_hero_section(data: dict):
    return f"""
    <style>
        @media (max-width: 768px) {{
            .mobile-breadcrumb {{
...
async def get_case_study_content(data: dict):
    content = []
    for i in data:
        if i["type"] == "text":
            content.append(
...
async def get_case_study_body(data: dict):
    hero_section = await get_case_study_hero_section(data)
    mobile_toc = await generate_mobile_toc(data["dynamicSections"])
    desktop_toc = await generate_desktop_toc(data["dynamicSections"])
    case_study_content = await get_case_study_content(data["dynamicSections"])

    return f"""
        <style>
            @media (max-width: 768px) {{
                .mobile-case-study-grid {{
                    margin-left: 0 !important;
                    margin-right: 0 !important;
        ...
        {hero_section}
        {mobile_toc}
        <div class="mobile-case-study-grid grid grid-cols-1 lg:grid-cols-[300px_1fr] md:ml-[-16rem] gap-8 mt-4 md:mt-8 ml-0 mr-0" style="padding: 0 0.5rem">
        {desktop_toc}
        {case_study_content}
        </div>
...
EMPTY_CASE_STUDY_TEMPLATE = """<!DOCTYPE html>
...
async def create_case_study_html(data: str) -> str:
    l = []
    l.append(await getHeader())
    l.append(await get_case_study_body(data))
    l.append(await get_more_case_studies_section(data))
    l.append(await get_faq_section())
    l.append(await getFooter())

    l = EMPTY_CASE_STUDY_TEMPLATE.replace("[[total_body]]", "\n".join(l))

    return l


@router.get("/case_study/{slug}")
async def get_case_study(slug: str):
    """
    Render a case study page by its slug
    Fetches case study data from database and renders it as HTML
    """
    print("=" * 80)
    print(f"[DEBUG] Case Study endpoint called with slug: {slug}")
    print("=" * 80)
    ...
    try:
        ...
        print(f"[DEBUG] Querying for case study with slug: {slug}")
        ...
        if not case_study_record:
            print(f"[DEBUG] Case Study not found for slug: {slug}")
            raise HTTPException(status_code=404, detail=f"Case Study not found: {slug}")
        
        print(f"[DEBUG] Case Study found - Status: {case_study_record['status']}")
        
        if case_study_record['status'] != 'published':
            print(f"[DEBUG] Case Study is not published, status: {case_study_record['status']}")
            raise HTTPException(status_code=404, detail="Case Study not available")
        
        case_study_data = case_study_record['case_study'] if isinstance(case_study_record['case_study'], dict) else json.loads(case_study_record['case_study'])
        print(f"[DEBUG] Rendering case study HTML...")
        
        html_content = await create_case_study_html(case_study_data)
...
@router.post("/api/admin_case_study_preview")
async def admin_case_study_preview(request: Request):
    """
    Preview case study from admin panel
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Preview data received, rendering preview.",
            "data": await create_case_study_html(data)
        }
        
    except Exception as e:
        print(f"✗ Error in admin_case_study_preview: {e}")
